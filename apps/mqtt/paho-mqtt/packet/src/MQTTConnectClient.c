...
int MQTTSerialize_connect(unsigned char* buf, int buflen, MQTTPacket_connectData* options)
{
	...
	header.byte = 0;
	header.bits.type = msgTypes_CONNECT;
...
int MQTTDeserialize_connack(unsigned char* sessionPresent, unsigned char* connack_rc, unsigned char* buf, int buflen)
{
	...
	if (header.bits.type != msgTypes_CONNACK)
		goto exit;
...
{
	return MQTTSerialize_zero(buf, buflen, msgTypes_DISCONNECT);
}
...
{
	return MQTTSerialize_zero(buf, buflen, msgTypes_PINGREQ);
}
