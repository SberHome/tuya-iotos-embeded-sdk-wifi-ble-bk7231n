...
static int mqtt_subscribe(MQTT_CLIENT_T *c, const char *topicFilter, enum QoS qos)
{
    ...
    if (rc == msgTypes_SUBACK)      // wait for suback
    {
...
{
    debug_printf("deliverMesage, topicName: %s\n", topicName->cstring);
    int i;
    int rc = PAHO_FAILURE;
...
static int mqtt_cycle(MQTT_CLIENT_T *c)
{
    ...
    switch (packet_type)
    {
    case msgTypes_CONNACK:
    case msgTypes_PUBACK:
    case msgTypes_SUBACK:
        break;
    case msgTypes_PUBLISH:
    {
        ...
        {
            if (msg.qos == QOS1)
                len = MQTTSerialize_ack(c->buf, c->buf_size, msgTypes_PUBACK, 0, msg.id);
            else if (msg.qos == QOS2)
                len = MQTTSerialize_ack(c->buf, c->buf_size, msgTypes_PUBREC, 0, msg.id);
            if (len <= 0)
                rc = PAHO_FAILURE;
            else
                rc = sendPacket(c, len);
    ...
    case msgTypes_PUBREC:
    {
        unsigned short mypacketid;
        unsigned char dup, type;
        if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
            rc = PAHO_FAILURE;
        else if ((len = MQTTSerialize_ack(c->buf, c->buf_size, msgTypes_PUBREL, 0, mypacketid)) <= 0)
            rc = PAHO_FAILURE;
    ...
    case msgTypes_PUBCOMP:
        break;
    case msgTypes_PINGRESP:
...
int mqtt_publish_with_topic(MQTT_CLIENT_T *c, const char *topicName, MQTTMessage *message)
{
    ...
    debug_printf("MQTTPublish sendto: %s, len: %d\n", topicName, len);
