...
static void dhcp_clean_sockets(void)
{
	...
	if (dhcp_nack_dns_server_handler)
	{
		if (dhcps.dnssock != -1)
		{
			...
			if (ret != 0)
			{
				dhcp_w("Failed to close dns port socket\r\n");
			}
			dhcps.dnssock = -1;
		}
	}
...
void dhcp_server(void *data)
{
	...
	while (1)
	{
		...
		if (dhcp_nack_dns_server_handler)
		{
			...
			max_sock = (dhcps.sock > dhcps.dnssock ? dhcps.sock : dhcps.dnssock);
		}

		FD_SET(dhcps.ctrlsock, &rfds);
		max_sock = (dhcps.sock > dhcps.ctrlsock ? dhcps.sock : dhcps.ctrlsock);
