...
int bk_wlan_dtim_rf_ps_mode_do_wakeup()
{
    ...
    if ((power_save_if_ps_rf_dtim_enabled() && power_save_if_rf_sleep()) || ble_switch_mac_sleeped)
    {
...
int bk_wlan_dtim_rf_ps_disable_send_msg(void)
{
    ...
    GLOBAL_INT_DISABLE();
    if (power_save_if_ps_rf_dtim_enabled() && power_save_if_rf_sleep())
...
{
    if (!bk_wlan_has_role(VIF_AP) && !bk_wlan_has_role(VIF_MESH_POINT) && !bk_wlan_has_role(VIF_IBSS) && !bk_wlan_is_monitor_mode())
    {
        return 1;
    }
    else
